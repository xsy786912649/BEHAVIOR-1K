...
def replay_hdf5_file(hdf_input_path):
    ...
    ):  # Avoid copying if already in target folder
    ...
    if RUN_QA:
        ...
        metric_kwargs = dict(
            ...
            head_camera=env.external_sensors[
                f"external_sensor{len(env.external_sensors) - 1}"
            ],
        ...
        active_metrics_info = {
            metric_name: ALL_QA_METRICS[metric_name]
            for metric_name in COMMON_QA_METRICS
        }
        for metric_name, metric_info in active_metrics_info.items():
            ...
            init_kwargs = {
                arg: metric_kwargs[arg] for arg in extract_arg_names(create_fcn)
            }
